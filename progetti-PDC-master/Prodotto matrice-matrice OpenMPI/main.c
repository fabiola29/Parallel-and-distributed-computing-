...
int main(int argc, char** argv) {
    ...
    // Se il numero di processori non è una radice quadrata di interi allora non è rappresentabile in una griglia pxp
    if(!isPerfectSquare(numberOfProcessors)) {
        if(processId == MASTER_ID) {
            fprintf(stderr, "ERROR - Number of processors provided cannot be distribuited in a NxN grid.\n"
    ...
    // Verifico la correttezza della dimensione della matrice passata in input
    if(!parseInt(argv[1], &matrixSize)) {
        fprintf(stderr, "ERROR - Cannot parse the matrixSize with value provided.\n"
                        "matrixSize:%s\n\n", argv[1]);
        MPI_Finalize();
        return 1;
    }

    // MatrixSize dev'essere uguale a numberOfProcessors o un suo multiplo
    if(matrixSize < numberOfProcessors || matrixSize % numberOfProcessors != 0) {
        fprintf(stderr, "ERROR - Matrix size must be >= of the grid and a multiple of number of processors.\n"
